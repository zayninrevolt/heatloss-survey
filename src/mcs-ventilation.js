(function (root, factory) {
  ...
  // MCS-reference ventilation model, built from the published reference data:
  // - air permeability default and mechanical system rates:
  //   https://heatloadcalculator.mcscertified.com/docs/reference-sources/ventilation-rates
  // - 50 Pa to typical-pressure conversion (divide by 20) and device rates:
  ...
  //
  // NOT the same method as the legacy workbook ACH table, and not yet validated
  // against an external calculator. Do not enable it for live surveys until that
  // comparison exists.

  var METHOD_ID = 'mcs-reference-ventilation-v1';
  var DEFAULT_PERMEABILITY_50 = 12;
  var PERMEABILITY_TO_INFILTRATION_DIVISOR = 20;
  ...
  function buildingInfiltration(input) {
    ...
    var flow = permeability50 / PERMEABILITY_TO_INFILTRATION_DIVISOR * envelopeArea;
    return {
      methodId: METHOD_ID,
