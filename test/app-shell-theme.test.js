...
test('uses the shared warm dark app-shell palette without styling survey sheets', () => {
  ...
  assert.doesNotMatch(appShellCss, /\.app-shell-theme\s+\.sheet(?:\s|,|\{|\.)/);
  assert.doesNotMatch(appShellCss, /\.app-shell-theme\s+\.sheet-wrap(?:\s|,|\{|\.)/);
});
